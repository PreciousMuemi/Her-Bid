...
export const HederaProvider = ({ children }: { children: ReactNode }) => {
  ...
  const connectToHedera = async (operatorId?: string, operatorKey?: string) => {
    try {
      ...
      // Get initial balance
      await fetchBalance(hederaClient, accountId);
  ...
  const initializeClient = () => {
    try {
      ...
      // Get initial balance
      fetchBalance(newClient, operatorId);
    } catch (error) {
  ...
  const fetchBalance = async (client: Client, id: string) => {
    try {
  ...
  const refreshBalance = async () => {
    ...
    await fetchBalance(client, accountId);
  };
  ...
  const sendHbarToMetaMask = async (toAddress: string, amount: number) => {
    ...
    try {
      ...
      const transferHbarTransactionSigned = await transferHbarTransaction.sign(PrivateKey.fromString(privateKey));
      ...
      if (transactionReceipt.children && transactionReceipt.children.length > 0) {
        ...
        if (childReceipt.accountId) {
          console.log(`New account created: ${childReceipt.accountId.toString()}`);
        }
      }
  ...
  const transferTokens = async (tokenId: string, receiverId: string, amount: number) => {
    ...
    try {
      const transaction = await new TransferTransaction()
        .addTokenTransfer(TokenId.fromString(tokenId), AccountId.fromString(accountId), -amount)
